...
def _git_tree(path):
    """Tracked and new, non-ignored files of a checkout, including work before the first commit."""
    if not os.path.isdir(os.path.join(path, ".git")):
        return None
    try:
        res = subprocess.run(["git", "ls-files", "--cached", "--others", "--exclude-standard"], cwd=path,
