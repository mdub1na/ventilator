...
`integrated-probe.sh prepare-reboot` makes an otherwise identical signed test app in the ignored `.reboot-probes/` directory, which remains available across a Mac restart. After `register` and `check`, run `reboot-before "$APP"`; it records the boot time and the UID 0 daemon's startup audit. Reboot the Mac yourself, then run `reboot-after "$APP"`. The latter requires a new boot time, an enabled registration, a valid startup audit with `control_allowed=false` from the root daemon and a fresh system-state read. A PID may be reused after reboot, so boot time is the identity check. Always finish with `unregister` and `cleanup`, then restore the background switch. This is a read-only lifecycle check, not a fan-control or SMC recovery test.
