...
| Две ad hoc подписанные копии с разными `cdhash` на `Mac15,7`/macOS 27.0 проверили требования подписи в обе стороны: доверенная пара обменялась статусом, другая копия клиента не дошла до delegate, а клиент отклонил другую копию сервиса с XPC ошибкой `4102`. Локальный поиск показал `0 valid identities found`. | [Протокол](../features/helper-status-ipc.md), `prototype/helper-ipc/helper-status.m`, `prototype/helper-ipc/smoke.sh`; проверка 2026-09-26 | API проверки подписи работают в пользовательской пробе. Ad hoc `cdhash` можно повторить копированием бинарника; Team ID и допуск только нашего приложения не доказаны. |
...
1. **Read-only пакет и IPC.** Пользовательский XPC обмен фиксированным статусом и регистрация отдельного тестового LaunchAgent через `SMAppService.agent` проверены. Далее собрать пакет Ventilator без SMC writer, зарегистрировать daemon через `SMAppService.daemon(plistName:)` после одобрения администратора. Проверить `notRegistered`/`requiresApproval`/`enabled`, обмен фиксированным снимком и полное удаление регистрации. Это ещё не M2-02 целиком.
2. **Проверка клиента.** Механизм двустороннего требования `cdhash` проверен только на временных ad hoc копиях. Далее собрать пакет со стабильной подписью, закрепить требование к доверенному подписанту и идентификатору на обеих сторонах XPC, подтвердить успешное подключение Ventilator и отказ неподписанному/чужому клиенту. Ad hoc сборка из §1 для доказательства этого критерия недостаточна.
