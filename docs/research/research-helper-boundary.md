...
3. **Жизненный цикл без записи.** `integrated-probe.sh restart-check` завершает только тестовый read-only daemon и проверяет новый PID, UID 0 и ответ на новый XPC запрос. `sleep-before/sleep-after` сверяют цикл сна/пробуждения в пределах одной загрузки и доступность статуса после него. Эти шаги ещё требуют ручной проверки на Mac. Позже отдельно проверить прерванный запрос в существующем соединении и выход UI. Не предполагать, что launchd или обработчик сигнала гарантирует мгновенный возврат SMC.
