...
1. **Read-only пакет и IPC.** Отдельный подписанный LaunchDaemon прошёл `requiresApproval → enabled → notRegistered`, root XPC ответ и проверенное удаление. Далее встроить ограниченный daemon в подписанный `Ventilator.app` без SMC writer и повторить регистрацию/удаление уже из настоящего пакета. Это ещё не M2-02 целиком.
2. **Проверка клиента.** В отдельном пакете обе стороны требуют Apple anchor, точный identifier и Team ID; ad hoc клиент и клиент с тем же Team ID, но другим identifier, отклонены. Далее подключить настоящий подписанный `Ventilator.app`. Developer сертификат подходит для локальной разработки; распространение и нотарификация здесь не проверены.
...
- Достаточно ли текущего правила подписи для интеграции с настоящим `Ventilator.app` и распространения. Локальный Apple Development сертификат и отдельный пакет проверены; Developer ID, распространение и нотарификация ещё нет. Личный Team ID не хранить в репозитории.
...
Отдельный привилегированный бинарник предоставляет только фиксированный статус. Моста Compose/JVM → XPC и аппаратных операций в нём нет; эта проба не доказывает безопасность будущего интерфейса записи.
