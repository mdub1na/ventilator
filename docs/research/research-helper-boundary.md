...
1. **Read-only пакет и IPC.** Собрать daemon без SMC writer, зарегистрировать через `SMAppService.daemon(plistName:)` после одобрения администратора. Проверить `notRegistered`/`requiresApproval`/`enabled`, обмен фиксированным снимком и полное удаление регистрации. Это ещё не M2-02 целиком.
...
Кода будущего daemon и XPC-моста в репозитории пока нет. Этот документ задаёт ограничения для их первой read-only версии, не описывает реализованный сервис.
