...
- `TemperatureReading` хранит сырой ключ, AppleSMC, °C, время обработки, доступность и уверенность подписи. `TCMz` — рабочая подпись максимума температуры кристалла CPU. `Tg0D` вырос примерно с 45 до 68 °C при короткой нагрузке GPU на этой модели и обозначен как датчик GPU. `TH0a` обозначен как предполагаемый датчик SSD по открытым картам SMC и остаётся с `RAW_KEY_ONLY`.
...
* **Manual:** в локальном `.app` на `Mac15,7` пользователь подтвердил расположение и читаемость карточек; связь `TH0a` с SSD ещё требует отдельного испытания.
...
SMC может сообщать `0 RPM` при системном управлении. Положительные RPM без доступного диапазона позволяют показать число оборотов, но не позволяют достоверно заполнить шкалу. Привязка `TH0a` к SSD на этой модели ещё не подтверждена отдельной нагрузкой накопителя, поэтому экран явно помечает её как предварительную. Старый изолированный AppKit прототип по-прежнему умеет читать JSON сам, но в составе `.app` запускается отдельный `status-item-bridge` без доступа к reader.
