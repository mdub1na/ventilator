...
Отдельный процесс отвечает на запрос статуса фиксированным снимком: версия протокола `1`, состояние `read_only_prototype`, `write_available=false`. Пользовательский ad hoc прототип сообщает `smc_access=false`; подписанный системный daemon теперь сообщает `smc_access=true` и предоставляет второй запрос для чтения фиксированных SMC-ключей. Запрос не принимает ключи или цели от клиента и не содержит операции записи. Daemon встроен только во временную подписанную копию настоящего `Ventilator.app`; обычный UI не запрашивает этот XPC сервис.

## 2. Правила

1. XPC-интерфейс содержит `fetchStatusWithReply` и `fetchBaselineWithReply`. Оба без параметров: клиент не передаёт ключ SMC, RPM, путь или команду.
2. Клиент принимает статус только с четырьмя ожидаемыми полями и версией `1`. Снимок принимается только при `available=true`, точной модели/ОС и фиксированной форме массивов. Ошибка или таймаут за 5 секунд означает недоступность сервиса.
...
10. Reader daemon ограничен `Mac15,7`/macOS 27.0, двумя вентиляторами и точными форматами SMC-ключей. При ошибке обязательного чтения он возвращает `available=false` с кодом причины, без подстановки нулевых показаний. Каждая конечная температура вне 10–115 °C становится `null` на своей позиции; клиент принимает `null`, но отвергает неправдоподобное число. Значение `baseline=true` означает только исходный снимок ключей управления (`Ftst=0`, режимы `[3,3]`, цели около нуля), а не восстановление после записи. Ключи читаются последовательно, поэтому снимок не является аппаратно атомарным и требует повторного наблюдения для вывода об устойчивости.
...
Проба `root-inflight-smoke.sh` на `Mac15,7`/macOS 27.0 использовала новую временную подписанную копию. После `requiresApproval` и повторной регистрации `check` подтвердил фиксированный ответ из JVM от UID 0 и отказ двух чужих клиентов. Пользователь запустил скрипт в Terminal с `sudo`: запрос оставался ожидающим при остановленном daemon, после `SIGKILL` завершился сообщением «Не удалось установить связь с приложением-помощником» без JSON-статуса (`interrupted-root-request=failed-closed`). Следующий запрос автоматически получил точный read-only статус от нового root PID: `old-pid=85675`, `new-pid=86052`, `recovery=automatic`. Независимая проверка показала `enabled`, новый PID с UID 0 в состоянии `Ss` и отсутствие старого PID. Затем `unregister` подтвердил `notRegistered` и отсутствие системной службы; пакет удалён, фоновый переключатель возвращён в «выкл.». Это проверка XPC при аварии daemon, без SMC доступа и без проверки восстановления вентиляторов.
...
### Scenario: Подписанный daemon отдаёт фиксированный снимок только для чтения
* **Given:** подписанный daemon запущен как временный пользовательский LaunchAgent на `Mac15,7`/macOS 27.0.
...
* **Manual:** `root-inflight-smoke.sh` между `integrated-probe.sh check` и `unregister/cleanup` на `Mac15,7`/macOS 27.0, §3; прерванный запрос завершился без статуса, новый ответил от другого root PID.
...
Этап проверил упаковку отдельного пользовательского agent, отдельного подписанного root daemon и временной подписанной копии настоящего `Ventilator.app`. Проверены XPC из главного JVM-процесса, двусторонние требования к подписи, отказ чужим клиентам и одноимённой пользовательской службе, прерывание запросов к пользовательскому и root daemon, новый ответ после `SIGKILL` root daemon, ответ после сна, авария UI и удаление служб. Не проверены обновление пакета, чтение или запись SMC из daemon и аппаратное восстановление. Успешный XPC ответ не закрывает M2-02. [Исследование границы](../research/research-helper-boundary.md) описывает следующий порядок.
