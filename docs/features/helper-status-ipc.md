...
Этап проверил упаковку отдельного пользовательского agent, отдельного подписанного root daemon и временной подписанной копии настоящего `Ventilator.app`. Проверены XPC из главного JVM-процесса, двусторонние требования к подписи, отказ чужим клиентам и одноимённой пользовательской службе, прерывание запроса к пользовательскому daemon, новый ответ после `SIGKILL` root daemon, ответ после сна и удаление служб. Не проверены обновление пакета, прерванный запрос к **root** daemon, авария UI, чтение или запись SMC из daemon и аппаратное восстановление. Успешный XPC ответ не закрывает M2-02. [Исследование границы](../research/research-helper-boundary.md) описывает следующий порядок.
