...
* **When:** диагностическая команда посылает `SIGKILL` именно этому daemon через `launchctl`, затем приложение делает новый запрос.
...
* **Manual:** `integrated-probe.sh restart-check` на `Mac15,7`/macOS 27.0; результат ожидает системного разрешения. Проверяется новый запрос, а не прерванный запрос в старом соединении.
...
* **Manual:** `integrated-probe.sh sleep-before/sleep-after` на `Mac15,7`/macOS 27.0; результат ожидает реального цикла сна и пробуждения.

## 5. Граница результата

Этап проверил упаковку отдельного пользовательского agent, отдельного подписанного root daemon и временной подписанной копии настоящего `Ventilator.app`. Проверены XPC из главного JVM-процесса, двусторонние требования к подписи, отказ чужим клиентам и удаление служб. Не проверены обновление пакета, чтение или запись SMC из daemon, реакция на аварии и сон, аппаратное восстановление. Успешный XPC ответ не закрывает M2-02. [Исследование границы](../research/research-helper-boundary.md) описывает следующий порядок.
