...
Этап проверил упаковку отдельного пользовательского agent, отдельного подписанного root daemon и временной подписанной копии настоящего `Ventilator.app`. Проверены XPC из главного JVM-процесса, двусторонние требования к подписи, отказ чужим клиентам, новый ответ после `SIGKILL` daemon, ответ после сна и удаление служб. Не проверены обновление пакета, прерванный запрос, авария UI, чтение или запись SMC из daemon и аппаратное восстановление. Успешный XPC ответ не закрывает M2-02. [Исследование границы](../research/research-helper-boundary.md) описывает следующий порядок.
...
| Копия основного приложения и JNI-мост | `prototype/helper-ipc/integrated-probe.sh`, `prototype/helper-ipc/HelperProbeBridge.m`, `prototype/desktop-app/src/main/kotlin/ventilator/desktop/helper/HelperProbeCommand.kt` |
