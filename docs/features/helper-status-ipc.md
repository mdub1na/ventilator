...
10. Reader daemon ограничен `Mac15,7`/macOS 27.0, двумя вентиляторами и точными форматами SMC-ключей. При ошибке он возвращает `available=false` с кодом причины, без подстановки нулевых показаний. Значение `baseline=true` означает только исходный снимок (`Ftst=0`, режимы `[3,3]`, цели около нуля), а не восстановление после записи. Ключи читаются последовательно, поэтому снимок не является аппаратно атомарным и требует повторного наблюдения для вывода об устойчивости.
