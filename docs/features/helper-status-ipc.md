...
* **Manual:** ожидает системного разрешения на `Mac15,7`/macOS 27.0; `integrated-probe.sh prepare/register/check/unregister/cleanup`.

## 5. Граница результата

Этап проверил упаковку отдельного пользовательского agent и отдельного подписанного root daemon, двусторонние требования к подписи и удаление обеих служб. Временная подписанная копия `Ventilator.app` с JNI-мостом собрана; XPC из неё пока ожидает разрешения. Не проверены обновление пакета, чтение или запись SMC из daemon и аппаратное восстановление. Успешный XPC ответ не закрывает M2-02. [Исследование границы](../research/research-helper-boundary.md) описывает следующий порядок.
