...
* **Manual:** после включения настройки и появления записи в «Открывать при входе» пользователь подтвердил автоматический запуск на `Mac15,7`: до первого щелчка по значку было видно только значок, окно оставалось скрытым. После выключения настройки и следующей перезагрузки приложение автоматически не запустилось.
