...
`make -C prototype/login-item build` использует Objective-C и заголовки JDK 25. Gradle включает библиотеку в каталог ресурсов локального `.app`. При `gradle run` служба не загружается и настройка недоступна: этот путь не запускает установленный app bundle. Библиотека загружается перед инициализацией Compose/AppKit, чтобы успеть увидеть начальный Apple Event. Если признак запуска не пришёл за 3 секунды, приложение показывает окно, сохраняя доступность ручного запуска.

## 4. Local verification

На `Mac15,7`, macOS 27.0, локальный ad hoc подписанный `.app` открыл окно при ручном запуске. Включение переключателя перевело `SMAppService.status` в `enabled` и добавило Ventilator в системный список «Открывать при входе». Отключение фонового разрешения в настройках перевело состояние в `requiresApproval`; окно показало действие для открытия настроек. После возврата разрешения статус стал `enabled`. Выключение переключателя убрало Ventilator из списка объектов входа. Первая перезагрузка без подтверждённой регистрации не запустила приложение; после повторного включения, проверки записи в системном списке и перезагрузки пользователь подтвердил автоматический запуск. До первого щелчка по значку окно оставалось скрытым. Отсутствие запуска после выключения настройки ещё не проверено при новом входе.
