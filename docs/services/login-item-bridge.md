...
На `Mac15,7`, macOS 27.0, локальный ad hoc подписанный `.app` открыл окно при ручном запуске. Включение переключателя перевело `SMAppService.status` в `enabled` и добавило Ventilator в системный список «Открывать при входе». Отключение фонового разрешения в настройках перевело состояние в `requiresApproval`; окно показало действие для открытия настроек. После возврата разрешения статус стал `enabled`. После включения, проверки записи в системном списке и перезагрузки пользователь подтвердил автоматический запуск; до первого щелчка по значку окно оставалось скрытым. После выключения настройки и следующей перезагрузки Ventilator автоматически не запустился.
