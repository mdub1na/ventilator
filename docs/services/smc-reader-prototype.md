...
Снимок `FNum`, `F{n}Ac`, `F{n}Mn`, `F{n}Mx`, `TCMz`, `Tg0D` и `TH0a` получают из локального AppleSMC без `sudo`. На `Mac15,7` связь `Tg0D` с GPU и `TH0a` с температурной зоной SSD проверена отдельными нагрузками. C-утилита выводит JSON, Kotlin/JVM запускает её и строит типизированный снимок. Отдельная команда по-прежнему перечисляет все температурные ключи для исследования, но основной экран её не вызывает. Это процессы прототипа на одной модели, не установленный системный сервис и не API управления вентиляторами. В этом модуле записи нет; экспериментальный selector записи находится только в отдельном [инструменте M2-01](smc-write-trial.md).
...
`smc-read` читает локальное SMC, проверяет тип и значение, выводит `null` при недоступности ключа. Kotlin запускает утилиту по переданному или включённому в `.app` пути, ждёт до 5 секунд для короткого снимка, проверяет код выхода и схему, затем фиксирует время обработки результата. Отдельное исследовательское перечисление имеет таймаут 20 секунд. При ошибке процесс завершается исключением, а не выдаёт `0 RPM`. Подписанное распространение пакета и защита от подмены reader ещё не проверены.
...
Reader и JVM-процесс могут работать вручную; Compose Gradle сборка также создаёт локальный `.app` с вложенными исполняемыми `smc-read` и `status-item-bridge`, JNI-библиотекой автозапуска и Java runtime. Kotlin/JVM один опрашивает короткий снимок и передаёт его AppKit значку по локальному stdin/stdout. Полная диагностика доступна как команда reader для исследований, в приложении её раздела нет. На `Mac15,7` запуск, чтение и наличие обоих процессов проверены. Автозапуск отдельно описан в [документе моста](login-item-bridge.md); подпись Developer ID, нотарификация, устойчивое распространение и привилегированный helper остаются отдельными этапами.
