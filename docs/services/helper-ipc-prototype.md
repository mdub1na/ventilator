...
Дополнительные команды `restart-check` и `sleep-before/sleep-after` относятся только к временной read-only копии. Первая завершает тестовый daemon через `launchctl kill SIGKILL`, затем проверяет новый root PID и новый XPC ответ. Вторая пара требует подтверждённого системой цикла сна/пробуждения в пределах одной загрузки, действующей регистрации и ответа после пробуждения. На текущем запуске эти сценарии ещё ждут разрешения фонового объекта и ручного пробуждения Mac; по ним нельзя делать вывод о восстановлении SMC.
