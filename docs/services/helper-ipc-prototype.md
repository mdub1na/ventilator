...
Отдельный [reducer будущего владения](../features/helper-control-lease.md) симулирует исходное минутное наблюдение, одного владельца с 15-секундным сроком и обязательную повторную проверку после потери клиента, сна и перезапуска daemon. Изолированный журнал фиксирует намерение до будущей записи и позволяет новому процессу отличить незавершённую пробу от чистого старта. Очистка маркера требует одноразовое подтверждение полного цикла восстановления в модели. Оба модуля собираются только в тестах; в `daemon-status` не линкуются и не добавляют XPC API или SMC запись.
...
Метод `fetchStatusWithReply` возвращает четыре поля: `protocol_version: 1`, `state: read_only_prototype`, `smc_access` и `write_available: false`. Для ad hoc сервера `smc_access=false`, для подписанного daemon с read-only reader — `true`. Метод `fetchBaselineWithReply` не принимает параметров; daemon читает только `FNum`, `Ftst`, режимы, цели и фактические RPM двух вентиляторов, `TCMz`, `Tg0D`, `TH0a`. При ошибке обязательного чтения он возвращает `available=false` и ограниченный код причины, а клиент отвергает снимок. При успехе ответ содержит `available=true`, модель/ОС, монотонное время чтения, исходное состояние и фиксированные массивы значений. Каждая температура — число 10–115 °C либо `null` для конечного значения вне этого рабочего диапазона; клиент отвергает сырое неправдоподобное число. Исходное состояние: `Ftst=0`, режимы `[3,3]`, обе цели не выше 1 RPM; недоступная температура не меняет эту классификацию. Для ad hoc `serve` и `request` CLI требует 40-значный ожидаемый `cdhash` peer. Для подписанного клиента и daemon нужен 10-значный Team ID и точные идентификаторы. Ошибка XPC, другая версия, иное содержимое или ожидание более 5 секунд отклоняются клиентом. Это локальный протокол, не HTTP API.
...
После этого helper стал кодировать конечные температуры вне 10–115 °C как `null` в `fetchBaseline`, startup audit и watcher. Автоматические тесты с подставленным `Tg0D=-1,95 °C` проверили сохранение классификации ключей управления, `null` для GPU и отказ клиента принимать сырое отрицательное число. На `Mac15,7`/macOS 27.0 обновлённый direct reader без записи получил `Ftst=0`, режимы `[3,3]`, цели `[0,0]` и температуры `[50,11; 43,74; 29,85] °C`; отдельная подписанная XPC проба в пользовательском домене вернула нормальные температуры `[50,58; 43,04; 29,73] °C`, отклонила двух чужих клиентов и удалила службу с пакетом. Повторная проба непосредственно после загрузки с новой версией не проводилась.
...
| `prototype/helper-ipc/ControlIntentJournal.c`, `prototype/helper-ipc/ControlIntentJournalTest.c` | изолированный устойчивый маркер намерения и тесты повторного открытия/повреждения; не входят в daemon |
