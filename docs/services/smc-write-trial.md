...
Если `Ftst=1` принят транспортом, но немедленное чтение не показывает `1`, state machine теперь выдаёт `TRIAL_RUN_WRITE_EFFECT_UNVERIFIED`, даже когда последующая попытка восстановления прочитала исходное состояние. CLI выводит `CRITICAL` и отдельный readback вместо сообщения о подтверждённом возврате. Симуляция применяет эффект записи уже после возврата функции; это не подтверждает аппаратное восстановление и не снимает блокировку `--apply`.
...
Дополнительный unit-тест моделирует принятую запись `Ftst=1`, которая остаётся невидимой во всех немедленных чтениях и меняет состояние позднее. State machine возвращает `TRIAL_RUN_WRITE_EFFECT_UNVERIFIED`; `make build test` проходит без аппаратной записи. Это регрессия для формулировки результата, а не доказательство возврата в системный режим.
