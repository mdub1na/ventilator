...
Инструмент не входит в Ventilator.app, не является привилегированным helper и не принимает произвольные ключи, типы, байты или RPM. Текущая версия не записывает `Ftst`/`FS! ` и не реализует unlock thermal manager. Прямая аппаратная проба 2026-09-25 дошла до записи `F0Md`, которую SMC отклонил `0x82`; переход в ручной режим не был подтверждён.
...
Обработчики `SIGINT`/`SIGTERM`/`SIGQUIT` направляют контролируемое завершение в восстановление, но процесс не может защититься от `SIGKILL`, аварии ядра или потери питания. Сон, crash и внешний watchdog относятся к M2-02. Если прямой переход отклонён, инструмент проверяет сохранность системного состояния и завершает опыт; он не пробует `Ftst` автоматически. Причина `0x82` на этой машине не установлена.
