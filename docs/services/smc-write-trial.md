...
`--apply` требует root; `trial --apply` дополнительно требует интерактивный TTY и повторный ввод фразы после свежего preflight. Неизвестный аргумент прекращает процесс до открытия пути записи.
